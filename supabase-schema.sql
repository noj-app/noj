...
-- 1,250 loyalty points, tier 'ذهبي'. Merchants deliberately kept to exactly
-- one example per active sector (مطاعم/بقالة/مقاهي/أخرى — plus the health
-- center seeded separately in supabase-migration-health-center.sql) rather
-- than several near-duplicates per sector; مواصلات is not offered at all
-- and never appears anywhere. 7 invoices here (plus one each from the
-- car-wash and health-center merchants seeded in later migration files —
-- 9 total system-wide) totaling 1,672.50 SAR split ~43% مطاعم / ~28%
-- بقالة / ~11% مقاهي / ~3% أخرى / ~15% صحة. Any other phone number just
-- gets a fresh, empty profile.
